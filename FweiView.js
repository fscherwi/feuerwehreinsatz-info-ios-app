...
export default class FweiView extends Component<Props> {
  ...
  onShouldStartLoadWithRequest(navState) {
    ...
    if (
      navState.url.indexOf('//' + baseURL.toLowerCase()) === -1 &&
      navState.url.indexOf('about:blank') === -1
    ) {
  ...
  render() {
    ...
    return (
      <View style={{flex: 1}}>
        <WebView
          source={{
            uri: (baseURL === 'about:blank' ? '' : 'https://') + baseURL,
          }}
          onShouldStartLoadWithRequest={navState =>
          ...
          bounce={false}
          onMessage={event =>
            this.handleWebViewMessage(JSON.parse(event.nativeEvent.data))
          }
        />
      </View>
    );
