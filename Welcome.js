  ...
  render() {
    ...
    return (
      <View>
        ...
        {customServer || (
          <Picker
            ...
            onValueChange={(itemValue, itemIndex) =>
              this.setState({unsavedServer: itemValue})
            }>
            {defaultServer.map(server => (
              <Picker.Item
                label={server.caption}
                key={server.caption}
                value={server.url}
              />
            ))}
        ...
        <View style={styles.customServer}>
          ...
          {customServer && (
            <TextInput
              ...
              value={unsavedServer}
              onChangeText={update =>
                this.setState({unsavedServer: update.toLowerCase()})
              }
            />
          )}
        </View>
        ...
        <Button
          flex
          title="Server übernehmen"
          onPress={() => this.save({baseURL: unsavedServer})}
        />
        <Text style={styles.infoText}>
          Die Servereinstellung kann später jederzeit über das Optionsmenü
          geändert werden.
        </Text>
      </View>
    );
